...
Функция СтруктураСклоненияЧерезЗапросКСервису(СклоняемыйТекст, ПоказыватьСообщения) 
	...
	Исключение
		ЗарегистрироватьОшибкуСервисаСклонений(ОписаниеОшибки(), ПоказыватьСообщения);
		Возврат СтруктураСклонения;
	КонецПопытки;
	
	ТелоОтвета = Ответ.ПолучитьТелоКакСтроку();
	Если Ответ.КодСостояния <> 200 Тогда
		ЗарегистрироватьОшибкуСервисаСклонений(ТелоОтвета, ПоказыватьСообщения);
		Возврат СтруктураСклонения;
	КонецЕсли;
...
Процедура ЗарегистрироватьОшибкуСервисаСклонений(ОписаниеОшибки, ПоказыватьСообщения)
	
	ИмяСобытия = НСтр("ru = 'Вызов веб-сервиса склонения'; en = 'Declension web serice call'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при вызове сервиса склонения. Обратитесь к администратору.
                  ...
                  |%1';
                  |en = 'Error while calling declensions web service. Please contact your administrator.
                  |Technical details
                  |%1'"), 
			ОписаниеОшибки);
